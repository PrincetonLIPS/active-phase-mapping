...
#Seeds that finished for CAL
seeds=np.array([ 0, 10, 11, 12, 15, 16, 17, 20, 21, 22, 25, 26, 27, 30, 31, 35, 36,
...
for seed in seeds:
    ...
    iterations =int((knot_N-dimensions)*num_polymorphs)
    print(knot_N)
    alpha=[0,0.25,0.5]
    for i in range(num_polymorphs):
    ...
    for it in tqdm(range(iterations)):
        ...
        next_x, entropy_change = get_next_candidate_baseline(poly_dict[counter]['posterior'], poly_dict[counter]['params'], poly_dict[counter]['dataset'],
        poly_dict[counter]['designs'], design_space)
