...
def calc_expected_energy_or_entropy(pts=None,endpoint_indices=None,knot_N=None,pred_mean=None,pred_cov=None,design_space=None,num_samples=None,avg=False,entropy=False):
    tol=1e-3
    samples=sample_from_posterior(pred_mean,pred_cov,design_space,num_samples,envelopes=False)
...
def EIG_chaase_meanfield(composition, dataset, seed, index_dict, pred_mean, pred_cov,
    ...
    for x in range(num_y): #inner loop, ys.
        ...
        entropy=calc_expected_energy_or_entropy(pts=pts,endpoint_indices=endpoint_indices,knot_N=knot_N,pred_mean=pred_mean,pred_cov=pred_cov,
        design_space=design_space,num_samples=num_samples,entropy=True)
